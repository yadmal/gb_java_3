...
//    Чтобы иметь возможность сереализации объектов, класс должен подключить интерфейс Serializable (в нем нет методов, просто маркер)
    // Если какой-то класс наследуется от другого класса не помеченого как Serializable, то у этого родительского класса обязательно должен быть конструктор по умолчанию (дефолтный, без параметров), потому что он будет вызван при восстановлении объекта. Если же родитель тоже сереализуем, то таких ограничений нет
