...
public class mt2_1_FixedThreadPool {
    public static void main(String[] args) throws Exception{
        // Потоки можно создавать либо наследуясь от Thread, либо реализовывая интерфейс Runnable.
        // Но также можно пользоваться средствами ExecutorService.
        // Например Executors.newFixedThreadPool(4) будет создавать очередь из 4 потока, которым мы можем отдавать
        // задачи на выполнение и если этих потоков будет нехватать, то задачи будут становиться
        // в очередь и выполняться по мере освобождения потоков. Потоки будут активны до тех пор,
        // пока мы не вызовем shutdown.

        // Вопрос: в какой момент они создаются - в момент создания или в тот момент, когда приходит новая задача.
        // У Executors.newFixedThreadPool() есть два конструктора: в одном просто указывается число потоков,
        // а во втором можно передать ThreadFactory. Когда пулу потоков потребуется новый поток,
        // он будет использовать эту фабрику для его создания.
        ...
        ExecutorService pool = Executors.newFixedThreadPool(4, new ThreadFactory() {
            ...
            // создаваться в этой фабрике, задать то, что они будут являться демонами и обладать
        ...
        // Runnable. Если execute просто выполняет задачу, то submit еще и возвращает результат
        ...
        Future<String> out = pool.submit(new Callable<String>() {
            ...
            public String call() throws Exception {
                // если метод будет выполняться долго, то главный поток программы не продолжит выполнение,
...
//        out.cancel(); // отменить задачу, если это возможно.
        ...
        // если мы хотим подождать, чтобы работу прекратил один поток, то мы вызываем join, а когда нужно дождаться
        // завершения работы всех потоков из пула, то вызывается awaitTermination
        // pool.awaitTermination(1000, TimeUnit.SECONDS);

        // елси имеется список задач, которые нужно исполнять, то их можно передать в методы invokeAll или invokeAny
        // как только сервис находится в состояние shutdown и все потоки у него выключились, он переходим
        // в режим terminated и уничтожается
        pool.shutdown();
