...
public class mt2_7_Locks {
    public static void main(String[] args) {
        ...
        // если какой-то поток подходит к блоку synchronized, то ему некуда деваться и он останавливается в этом месте
...
*/
        ...
        // Помимо указанных отличий от блоков синхронизации у этой конструкции есть метод tryLock.
        // Это попытка засинхронинзироваться. Если замок свободен - то мы его занимаем и возвращаем true, иначе
        // вернется false. Также можно установить таймаут, что является своеобразной защитой от дэдЛока
        new Thread(new Runnable() {
            @Override
            public void run() {
                if (rl.tryLock()) { // <***********************
